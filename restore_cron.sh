...
if [[ ! -f "$CHECKSUM_FILE" ]]; then
    echo "[ERREUR] Empreinte introuvable : $CHECKSUM_FILE" >&2
    [[ "${CLEANUP_KEY:-0}" -eq 1 ]] && rm -f "$KEY_FILE"
    exit 1
fi

echo "Vérification de l'empreinte SHA-256..."
if ! (cd "$ARCHIVE_DIR" && sha256sum -c "$CHECKSUM_FILE"); then
    echo "[ERREUR] Altération détectée : l'empreinte SHA-256 ne correspond pas !" >&2
    [[ "${CLEANUP_KEY:-0}" -eq 1 ]] && rm -f "$KEY_FILE"
    exit 1
fi
echo "Empreinte OK."
